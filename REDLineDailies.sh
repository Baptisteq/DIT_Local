...
echo "$0 RAWREDFILE OUTPUTFILENAME PRCODEC COLORSCIENCE ROLLOFF OUTPUTTONEMAP GAMMACURVE COLORSPACE"
echo "ERROR: $ERROR" 
exit
}
...
WINPATHOUTPUTFILENAME=$(echo "$OUTPUTFILENAME" | sed "s/cygdrive\/[a-z]\///" | tr "/" "\\" 2> /dev/null )
...
[ "$RESIZE" != "4096" ] && [ "$RESIZE" != "3840" ] && [ "$RESIZE" != "2048" ] && [ "$RESIZE" != "1920" ] && [ "$RESIZE" != "1280" ] && RESIZE="1080" && echo "Resize value needs to be either 1080 or 720. When 1080 is set. A 1920x1080 resize is applied while preserving OAR. 
When 720 is set. A 1280x720 resize is applied while preserving OAR.
By default. A 1920x1080 resize has been applied."

# RESIZE PROCESS #
[ "$RESIZE" == "4096" ] && SETSIZE="--resizeX 4096 --resizeY 2160"
[ "$RESIZE" == "3840" ] && SETSIZE="--resizeX 3840 --resizeY 2160"
[ "$RESIZE" == "2048" ] && SETSIZE="--resizeX 2048 --resizeY 1080"
[ "$RESIZE" == "1920" ] && SETSIZE="--resizeX 1920 --resizeY 1080"
[ "$RESIZE" == "1280" ] && SETSIZE="--resizeX 1280 --resizeY 720"

#---REDline encode ----#
# predicts if the generated path is over 256 characters count. If so usage exit
GENERATEDFILECOUNT=$(echo "$OUTPUTFILENAME"".mov" | wc -c)
...
GENERATEDFILE="$OUTPUTFILENAME"".mov"
if [ -f $GENERATEDFILE ]; then
  echo $GENERATEDFILE
  else
"$REDLINE" --i $WINPATHRAWREDFILE --o $WINPATHOUTPUTFILENAME --useRMD 1 --start 0 --end 5 --format 201 --PRcodec $PRCODEC --gpuPlatform 0 --colorSciVersion $COLORSCIENCE --NR 0 --rollOff $ROLLOFF --outputToneMap $OUTPUTTONEMAP --gammaCurve $GAMMACURVE --colorSpace $COLORSPACE $SETSIZE
fi
